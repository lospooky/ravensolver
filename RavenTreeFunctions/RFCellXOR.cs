...
{
    public class RFCellXOR : RavenFunction
    {
        #region RavenFunction Members
        int numCells = -1;

        public RFCellXOR(Tree tree, SearchDirection allowedDirections, Boolean testMode) : base(tree, allowedDirections, testMode) {
        ...
        public override Object ProcessAttributeList(List<Object> attributeValues, int outerIterator) {
            ...
            for (int i=0; i<attributeValues.Count; i++) {
                if (i != numCells - 1) {
                    ...
                    foreach (Object cellObject in cellObjects) {
                        if (allCellObjects.Contains(cellObject))
                            allCellObjects.Remove(cellObject);
                        else
                            allCellObjects.Add(cellObject);
                    }
                }
            ...
            //Logging.logInfo("RFCellXOR succeeded for row " + outerIterator);
            return true;
        }
